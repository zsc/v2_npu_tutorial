...
实际设计考量：
- 电源完整性：需预留30-40%的bump用于电源/地
- 信号完整性：高速信号需要屏蔽，降低有效密度
- 热管理：功耗密度限制在300-500W/cm²
...
带宽密度优势：
- 2.5D封装：~2-4 TB/s/mm边缘
- 3D封装：~10-20 TB/s/mm²面积
- 延迟降低：从~5ns降至~0.5ns
...
状态转换的关键考虑：
- Modified (M)：独占且已修改，需要写回主存
- Exclusive (E)：独占但未修改，可无总线事务升级为M
- Shared (S)：多核共享只读副本
- Invalid (I)：无效或未缓存

NPU特定优化：
- **权重共享优化**：权重参数通常只读，可长期保持S状态
- **激活值流式处理**：采用write-through策略减少M状态
- **批量失效**：层间切换时批量invalidate，减少事务数
