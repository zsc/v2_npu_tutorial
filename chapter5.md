...
脉动阵列是一种规则的处理单元（PE）阵列结构，数据像心脏泵血一样有节奏地在PE间流动。每个PE在每个时钟周期执行相同类型的操作，通过精心设计的数据流动模式实现高效的并行计算。

脉动阵列的核心特征：
- **局部通信**：每个PE只与相邻PE通信，避免了长距离布线
- **规则结构**：PE排列规则，便于VLSI实现和扩展
- **同步计算**：所有PE在统一时钟下工作，简化控制逻辑
- **数据重用**：通过数据在PE间传递实现高效重用
...
TPU选择Weight-stationary设计的关键原因：

**优势分析**：
- **能效最优**：权重静止避免了大量数据移动，减少动态功耗
- **带宽效率**：权重预加载后可重复使用，适合批处理
- **控制简单**：权重地址生成逻辑简化，减少控制开销
...
脉动阵列的维度选择直接影响硬件利用率和性能。设阵列维度为 $M \times N$，分析不同场景下的利用率：

**矩阵乘法映射效率**：

对于矩阵乘法 $C_{P \times Q} = A_{P \times K} \times B_{K \times Q}$：
...
TPU v1是Google的第一代张量处理器，专为数据中心推理工作负载设计。
...
脉动阵列的控制方式直接影响其灵活性和效率。TPU采用了CISC风格的指令集，结合了多种控制模式的优点。
